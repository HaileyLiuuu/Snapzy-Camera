...
enum SnapzyConfigurationDefaultDocument {
  ...
  private static func writeRecording(_ writer: inout SimpleTOMLWriter) {
    ...
    writer.value("capture_camera", false)
    writer.value("camera_device_id", RecordingCameraDevice.systemPreferredID)
    writer.value("camera_shape", CameraOverlayShape.widescreen.rawValue)
    writer.value("camera_mirrored", false)
    ...
    writer.value("center_x", 0.0)
    writer.value("center_y", 0.0)
    writer.value("width", 0.0)
    writer.value("uses_default_placement", true)
